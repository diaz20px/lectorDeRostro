...
def funcion_reconocimiento_original():
    ...
    # El detector se abre dentro del hilo secundario de video
    with vision.FaceLandmarker.create_from_options(opciones_mediapipe) as detector:
        ...
        while app.camara_activa and app.ejecutandose:
            ...
            # Dibujar la nube de puntos biométricos reactivos (Verde si es válido / Rojo si es intruso)
            ...
            # Enviar el flujo continuo al widget de CustomTkinter
...
class VentanaEstilos(ctk.CTk):
    ...
    def arrancar_lector(self):
        ...
        if not self.camara_activa:
            ...
            # Limpiamos el texto para que la pantalla no se encime con el video
            self.lbl_pantalla_video.configure(text="")
    ...
    def parar_lector(self):
        """ FUNCIONALIDAD 2: DETENER """
        self.camara_activa = False
        self.lbl_status.configure(text="Estado: Inactivo", text_color="#555555")
        
        # Quitamos la última imagen de la cámara y restauramos el texto de aviso
